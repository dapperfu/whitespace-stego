        ...
        for key, value in kwargs.items():
            if value is not None:
                if key in ["message", "carrier", "password"]:
                    if key == "carrier" and impl_id == "rust_cli":
                        print(f"[DEBUG] Python test harness carrier repr: {repr(value)}")
                        print(f"[DEBUG] Python test harness carrier codepoints: {[hex(ord(c)) for c in str(value)[:40]]}")
