...
func TestDecodeAllInvalidMessagesWithPassword(t *testing.T) {
	...
	_, err := Decode(carrier, "password")
	if err == nil || !strings.Contains(err.Error(), "invalid password or no valid messages found") {
		t.Fatalf("Expected invalid password or no valid messages found error, got: %v", err)
	}
}
