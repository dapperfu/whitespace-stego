...
fn test_get_encoded_message_position() {
    ...
    // Convert byte positions to character positions for proper slicing
    let mut start_char_pos = 0;
    let mut end_char_pos = 0;
    let mut found_start = false;
    let mut found_end = false;

    for (char_pos, (byte_pos, _)) in encoded.char_indices().enumerate() {
    ...
    // Use character-based slicing to check the markers
    let encoded_chars: Vec<char> = encoded.chars().collect();
    let start_slice: String = encoded_chars[start_char_pos..].iter().collect();
    let end_slice: String = encoded_chars[..=end_char_pos].iter().collect();

    assert!(start_slice.starts_with(START_MARKER));
    assert!(end_slice.ends_with(END_MARKER));
}
...
fn test_has_encoded_message() {
    ...
    assert!(!has_encoded_message(&format!(
        "Text with start{}",
        START_MARKER
    )));
    assert!(!has_encoded_message(&format!(
