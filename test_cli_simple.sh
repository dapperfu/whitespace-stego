...
# Simple CLI Cross-Implementation Test Suite
# Tests key permutations between Python, Rust, and C implementations
...
TEST_CASES=(
    ...
    "Very long message that exceeds normal length and contains lots of text to test encoding and decoding capabilities|Very long carrier text that exceeds normal length and contains lots of text to test encoding and decoding capabilities|password|Long text"
    "Hello|Hello World||No password"
)

# Empty carrier test cases (only for Rust)
EMPTY_CARRIER_TESTS=(
    # Format: "message|password|description"
    "Hello|password|Empty carrier with password"
    "Hello||Empty carrier no password"
    "Hello, World! 🌍|secret123|Empty carrier Unicode"
)
...
test_cross_implementation() {
    ...
    print_status "INFO" "Testing cross-implementation: $encode_impl → $decode_impl"
    
    for test_case in "${TEST_CASES[@]}"; do
        IFS='|' read -r message carrier password description <<< "$test_case"
        test_roundtrip "$encode_impl" "$decode_impl" "$message" "$carrier" "$password" "$description"
    done
}
...
# Function to test empty carrier (Rust only)
test_empty_carrier() {
    print_status "INFO" "Testing empty carrier scenarios (Rust only)"
    
    for test_case in "${EMPTY_CARRIER_TESTS[@]}"; do
        IFS='|' read -r message password description <<< "$test_case"
        test_roundtrip "rust" "rust" "$message" "" "$password" "$description"
    done
}
...
main() {
    echo "=========================================="
    echo "  Simple CLI Cross-Implementation"
    echo "           Test Suite"
    echo "=========================================="
    echo
    ...
    for encode_impl in "${CLI_IMPLS[@]}"; do
        for decode_impl in "${CLI_IMPLS[@]}"; do
            if [[ "$encode_impl" != "$decode_impl" ]]; then
                test_cross_implementation "$encode_impl" "$decode_impl"
                echo
            fi
        done
    done
    
    # Test empty carrier scenarios (Rust only)
    test_empty_carrier
    echo
