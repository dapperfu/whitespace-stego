...
def main():
    ...
    try:
        ...
        run_command(['python3', '-m', 'whitespace_stego.cli', '--help'], "Main CLI help")
        run_command(['python3', '-m', 'whitespace_stego.cli', 'encode', '--help'], "Encode command help")
        run_command(['python3', '-m', 'whitespace_stego.cli', 'decode', '--help'], "Decode command help")
        
        # 2. Backend Validation
        print("\n🔧 SECTION 2: Backend Validation")
        run_command(['python3', '-m', 'whitespace_stego.cli', '--backend', 'python', 'encode', '--help'], 
                   "Python backend validation")
        run_command(['python3', '-m', 'whitespace_stego.cli', '--backend', 'rust', 'encode', '--help'], 
                   "Rust backend validation")
        run_command(['python3', '-m', 'whitespace_stego.cli', '--backend', 'invalid', 'encode', '--help'], 
                   "Invalid backend validation", expect_success=False)
        
        # 3. Mutually Exclusive Options
        print("\n🚫 SECTION 3: Mutually Exclusive Options")
        run_command(['python3', '-m', 'whitespace_stego.cli', 'encode', 
                    '--message', 'test', '--message-file', files['message'], 
                    '--carrier', 'carrier', '--output', 'test.out'], 
                   "Mutually exclusive message options", expect_success=False)
        
        run_command(['python3', '-m', 'whitespace_stego.cli', 'encode', 
                    '--message', 'test', '--carrier', 'carrier', 
                    '--carrier-file', files['carrier'], '--output', 'test.out'], 
                   "Mutually exclusive carrier options", expect_success=False)
        
        # 4. Missing Required Options
        print("\n❌ SECTION 4: Missing Required Options")
        run_command(['python3', '-m', 'whitespace_stego.cli', 'encode', '--output', 'test.out'], 
                   "Missing message and carrier options", expect_success=False)
        
        run_command(['python3', '-m', 'whitespace_stego.cli', 'encode', 
                    '--message', 'test', '--output', 'test.out'], 
                   "Missing carrier option", expect_success=False)
        
        # 5. Basic Encoding/Decoding
        print("\n🔐 SECTION 5: Basic Encoding/Decoding")
        run_command(['python3', '-m', 'whitespace_stego.cli', 'encode', 
                    '--message', 'Hello World!', '--carrier', 'This is a test carrier.'], 
                   "Encode message+carrier to stdout")
        
        run_command(['python3', '-m', 'whitespace_stego.cli', 'encode', 
                    '--message', 'File output test', '--carrier', 'Carrier for file', 
                    '--output', 'demo_encoded.txt'], 
                   "Encode to file")
        
        run_command(['python3', '-m', 'whitespace_stego.cli', 'decode', 
                    '--carrier-file', 'demo_encoded.txt'], 
                   "Decode from file to stdout")
        
        run_command(['python3', '-m', 'whitespace_stego.cli', 'decode', 
                    '--carrier-file', 'demo_encoded.txt', '--output', 'demo_decoded.txt'], 
                   "Decode to file")
        
        # 6. File Input/Output Combinations
        print("\n📁 SECTION 6: File Input/Output Combinations")
        run_command(['python3', '-m', 'whitespace_stego.cli', 'encode', 
                    '--message-file', files['message'], '--carrier', 'Command line carrier'], 
                   "Message from file, carrier from command line")
        
        run_command(['python3', '-m', 'whitespace_stego.cli', 'encode', 
                    '--message', 'Command line message', '--carrier-file', files['carrier']], 
                   "Message from command line, carrier from file")
        
        run_command(['python3', '-m', 'whitespace_stego.cli', 'encode', 
                    '--message-file', files['message'], '--carrier-file', files['carrier']], 
                   "Both message and carrier from files")
        
        # 7. Stdout Output Options
        print("\n📤 SECTION 7: Stdout Output Options")
        run_command(['python3', '-m', 'whitespace_stego.cli', 'encode', 
                    '--message', 'Stdout test', '--carrier', 'Stdout carrier', '--output', '-'], 
                   "Explicit stdout output with dash")
        
        # 8. Password Protection
        print("\n🔒 SECTION 8: Password Protection")
        run_command(['python3', '-m', 'whitespace_stego.cli', 'encode', 
                    '--message', 'Secret with password', '--carrier', 'Protected carrier', 
                    '--password', 'mypassword', '--output', 'demo_encoded_pwd.txt'], 
                   "Encode with password protection")
        
        run_command(['python3', '-m', 'whitespace_stego.cli', 'decode', 
                    '--carrier-file', 'demo_encoded_pwd.txt', '--password', 'mypassword'], 
                   "Decode with correct password")
        
        run_command(['python3', '-m', 'whitespace_stego.cli', 'decode', 
                    '--carrier-file', 'demo_encoded_pwd.txt', '--password', 'wrongpassword'], 
                   "Decode with wrong password", expect_success=False)
        
        # 9. Unicode and Emoji Support
        print("\n🌍 SECTION 9: Unicode and Emoji Support")
        run_command(['python3', '-m', 'whitespace_stego.cli', 'encode', 
                    '--message', 'Hello 世界 🌍', '--carrier', 'Unicode carrier: café naïve', 
                    '--output', 'demo_unicode.txt'], 
                   "Encode Unicode content")
        
        run_command(['python3', '-m', 'whitespace_stego.cli', 'decode', 
                    '--carrier-file', 'demo_unicode.txt'], 
                   "Decode Unicode content")
        
        # 10. Verbose Mode
        print("\n🔍 SECTION 10: Verbose Mode")
        run_command(['python3', '-m', 'whitespace_stego.cli', '--verbose', 'encode', 
                    '--message', 'Verbose test', '--carrier', 'Verbose carrier', '--output', '-'], 
                   "Verbose mode encoding")
        
        # 11. Short Options
        print("\n⚡ SECTION 11: Short Options")
        run_command(['python3', '-m', 'whitespace_stego.cli', '-b', 'python', 'encode', 
                    '-m', 'Short options test', '-c', 'Short carrier text', '-o', '-'], 
                   "All short options")
        ...
        encode_result = subprocess.run(['python3', '-m', 'whitespace_stego.cli', 'encode', 
                                       '--message', 'Pipeline test message', 
                                       '--carrier', 'Pipeline carrier text'], 
        ...
        run_command(['python3', '-m', 'whitespace_stego.cli', '--backend', 'rust', 'encode', 
                    '--message', 'test', '--carrier', 'carrier'], 
                   "Rust backend error (when not installed)", expect_success=False)
        ...
        cleanup_list = ['demo_encoded.txt', 'demo_decoded.txt', 'demo_encoded_pwd.txt', 'demo_unicode.txt']
