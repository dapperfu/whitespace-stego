...
class WhitespaceStegoTester:
    ...
    def run_command(
        self, cmd: List[str], description: str, expect_success: bool = True
    ) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                description, False, f"Exception: {type(e).__name__}", str(e)
            )
    ...
    def test_python_cli_help(self) -> TestResult:
        ...
        return self.run_command(
            ["python3", "-m", "whitespace_stego.cli", "--help"], "Python CLI main help"
        )
    ...
    def test_rust_cli(self) -> TestResult:
        ...
        help_result = self.run_command(
            ["./whitespace-stego-rs", "--help"], "Rust CLI help"
        )
        if not help_result.success:
            return help_result

        # Test encode help
        encode_help = self.run_command(
            ["./whitespace-stego-rs", "encode", "--help"], "Rust CLI encode help"
        )
    ...
    def test_c_cli(self) -> TestResult:
        ...
        if not os.path.exists("./whitespace-stego-c"):
            return TestResult(
                "C CLI", False, "C CLI not found", "Run 'make c' to build the C CLI"
            )
    ...
    def test_core_api(self) -> TestResult:
        ...
        try:
            ...
            if decoded == "Test message":
                return TestResult(
                    "Core API basic functionality", True, "Basic encode/decode working"
                )
            else:
                return TestResult(
            ...
            return TestResult("Core API", False, "Failed to import core module", str(e))
        except Exception as e:
            return TestResult(
                "Core API", False, f"API test failed: {type(e).__name__}", str(e)
            )
    ...
    def test_core_api_password_protection(self) -> TestResult:
        ...
        try:
            ...
            if decoded == "Secret message":
                return TestResult(
                    "Core API password protection", True, "Password protection working"
                )
            else:
                return TestResult(
    ...
    def test_core_api_unicode(self) -> TestResult:
        ...
        try:
            ...
            if decoded == "Hello 世界 🌍":
                return TestResult(
                    "Core API Unicode support", True, "Unicode support working"
                )
            else:
                return TestResult(
    ...
    def test_pytest_suite(self) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                "Pytest Test Suite", False, "Tests timed out after 60 seconds"
            )
        except Exception as e:
            return TestResult(
                "Pytest Test Suite",
    ...
    def print_results(self):
        ...
        if failed == 0:
            print(
                "🎉 ALL TESTS PASSED! The whitespace steganography project is fully functional."
            )
        else:
