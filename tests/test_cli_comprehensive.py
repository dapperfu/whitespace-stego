...
class TestCLIComprehensive:
    ...
    def temp_files(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            unicode_carrier_file.write_text("Unicode carrier: café naïve", encoding="utf-8")
    ...
    def test_encode_explicit_stdout(self, runner):
        ...
        result = runner.invoke(cli, [
            'encode',
            '--message', 'Test with dash',
            '--carrier', 'Carrier with dash',
            '--output', '-'
        ], catch_exceptions=False)
        assert result.exit_code == 0
        assert "Message encoded successfully" in result.output
    ...
    def test_decode_file_stdout(self, runner, temp_files):
        ...
        runner.invoke(cli, [
            'encode',
            '--message', 'Test message for decode',
            '--carrier', 'Carrier for decode test',
    ...
    def test_decode_explicit_stdout(self, runner, temp_files):
        ...
        runner.invoke(cli, [
            'encode',
            '--message', 'Test message for explicit stdout',
            '--carrier', 'Carrier for explicit stdout test',
            '--output', str(encoded_file)
        ], catch_exceptions=False)
    ...
    def test_decode_file_output(self, runner, temp_files):
        ...
        runner.invoke(cli, [
            ...
            '--carrier', 'Carrier for file output test',
            '--output', str(encoded_file)
        ], catch_exceptions=False)
        ...
        assert decoded_file.read_text(encoding="utf-8").strip() == "Test message for file output"
    ...
    def test_unicode_support(self, runner, temp_files):
        ...
        result = runner.invoke(cli, [
            'encode',
            '--message', 'Hello 世界 🌍',
            '--carrier', 'Unicode carrier: café naïve',
            '--output', str(encoded_file)
        ], catch_exceptions=False)
        assert result.exit_code == 0
        assert "Message successfully encoded into" in result.output
    ...
    def test_roundtrip_pipeline_simulation(self, runner):
        ...
        try:
            ...
            decode_result = runner.invoke(cli, [
                'decode',
                '--carrier-file', temp_encoded_file
            ], catch_exceptions=False)
    ...
    def test_all_long_options(self, runner):
        ...
        result = runner.invoke(cli, [
            '--backend', 'python',
            'encode',
            '--message', 'Long options test',
            '--carrier', 'Long carrier text',
            '--output', '-'
        ], catch_exceptions=False)
        assert result.exit_code == 0
        assert "Message encoded successfully" in result.output 