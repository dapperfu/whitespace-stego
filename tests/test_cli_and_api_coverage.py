...
def test__decode_python_truncated_message():
    ...
    bits = "".join(
        encode_mod.ZERO_BIT if b == "0" else encode_mod.ONE_BIT for b in length_bits
    )
    text = encode_mod.START_MARKER + bits + encode_mod.END_MARKER
    with pytest.raises(ValueError):
        decode_mod._decode_python(text)
...
def test__decode_python_corrupted_base64():
    ...
    bits = "".join(
        encode_mod.ONE_BIT if b == "1" else encode_mod.ZERO_BIT for b in all_binary
    )
    text = encode_mod.START_MARKER + bits + encode_mod.END_MARKER
...
def test__encode_python_empty_carrier():
    ...
    assert (
        encoded
        == encode_mod.START_MARKER
        + encoded[len(encode_mod.START_MARKER) : -len(encode_mod.END_MARKER)]
        + encode_mod.END_MARKER
    )
...
def test_decode_python_general_exception():
    ...
    invalid_encoded_data = "".join(
        encode_mod.ONE_BIT if b == "1" else encode_mod.ZERO_BIT for b in invalid_binary
    )
...
def test_decode_python_password_exception():
    ...
    with pytest.raises(
        ValueError, match="Message binary length is not a multiple of 8"
    ):
        decode_mod._decode_python(corrupted, password)
