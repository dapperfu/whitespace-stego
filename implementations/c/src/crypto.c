...
int crypto_encrypt_static(const unsigned char* data, size_t data_len,
    ...
    if (!data || !password || !result_buffer || !result_len) {
        return 0;
    }
    
    // Check if buffer is large enough
    size_t required_size = IV_LEN + data_len + EVP_MAX_BLOCK_LENGTH;
    if (required_size > result_buffer_size) {
        return 0;
    }
    ...
    // Copy IV to output
    memcpy(result_buffer, iv, IV_LEN);
    int out_len = IV_LEN;
    
    // Encrypt data
    int len;
    if (EVP_EncryptUpdate(ctx, result_buffer + out_len, &len, data, data_len) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        return 0;
    }
    out_len += len;
    ...
    // Finalize encryption
    if (EVP_EncryptFinal_ex(ctx, result_buffer + out_len, &len) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        return 0;
    }
    out_len += len;
...
int crypto_decrypt_static(const unsigned char* data, size_t data_len,
                          const char* password, unsigned char* result_buffer,
                          size_t result_buffer_size, size_t* result_len) {
    if (!data || !password || !result_buffer || !result_len || data_len < IV_LEN) {
        return 0;
    }
    ...
    // Check if buffer is large enough
    if (encrypted_len > result_buffer_size) {
        return 0;
    }
    ...
    // Decrypt data
    int out_len;
    if (EVP_DecryptUpdate(ctx, result_buffer, &out_len, encrypted, encrypted_len) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        return 0;
    }
    ...
    // Finalize decryption
    int len;
    if (EVP_DecryptFinal_ex(ctx, result_buffer + out_len, &len) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        return 0;
    }
