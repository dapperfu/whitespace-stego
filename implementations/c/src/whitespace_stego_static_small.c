 ...
 * 
 * This implementation uses only static memory allocation with a 1 MiB character limit.
...
int whitespace_stego_static_small_validate_size(size_t size) {
    return (size <= MAX_BUFFER_SIZE_SMALL) ? 1 : 0;
}

size_t whitespace_stego_static_small_max_size(void) {
    return MAX_BUFFER_SIZE_SMALL;
}
...
static int encode_binary_static_small(const unsigned char* data, size_t data_len, 
...
static int decode_binary_static_small(const char* encoded, unsigned char* out_buffer, 
...
int whitespace_stego_static_small_encode(whitespace_stego_buffers_small_t* buffers,
    ...
    if (!whitespace_stego_static_small_validate_size(carrier_len) ||
        !whitespace_stego_static_small_validate_size(strlen(message))) {
    ...
    size_t zw_len = 0;
    if (!encode_binary_static_small((const unsigned char*)buffers->base64, buffers->base64_len, 
...
int whitespace_stego_static_small_decode(whitespace_stego_buffers_small_t* buffers,
    ...
    if (!whitespace_stego_static_small_decode_all(buffers, carrier, carrier_len, password, &result_count)) {
        return 0;
    }
...
int whitespace_stego_static_small_decode_all(whitespace_stego_buffers_small_t* buffers,
    ...
    size_t start_positions[MAX_MESSAGES_SMALL];
    size_t end_positions[MAX_MESSAGES_SMALL];
    ...
    while (*p && marker_count < MAX_MESSAGES_SMALL) {
    ...
    for (size_t i = 0; i < marker_count && buffers->message_count < MAX_MESSAGES_SMALL; i++) {
        ...
        if (!decode_binary_static_small(buffers->temp, (unsigned char*)buffers->base64, 
        ...
        if (password && password[0]) {
            ...
            if (message_len >= MAX_BUFFER_SIZE_SMALL) {
                continue; // Skip if message too large
            }
            
            memcpy(buffers->message + buffers->message_count * MAX_BUFFER_SIZE_SMALL, 
                   buffers->temp, message_len + 1);
            buffers->message_array[buffers->message_count] = 
                buffers->message + buffers->message_count * MAX_BUFFER_SIZE_SMALL;
            ...
            buffers->message[buffers->message_count * MAX_BUFFER_SIZE_SMALL + message_len] = '\0';
            buffers->message_array[buffers->message_count] = 
                buffers->message + buffers->message_count * MAX_BUFFER_SIZE_SMALL;
            buffers->message_count++;
        }
