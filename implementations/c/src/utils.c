...
int to_base64_static(const unsigned char* data, size_t data_len, 
    ...
    if (!data || !result_buffer || !result_len || data_len == 0) {
        return 0;
    }

    // Calculate output size
    size_t out_len = ((data_len + 2) / 3) * 4 + 1;
    if (out_len > result_buffer_size) {
        return 0;
    }
    ...
    for (i = 0, j = 0; i < data_len; i += 3, j += 4) {
        ...
        result_buffer[j] = b64_table[(triple >> 18) & 0x3F];
        result_buffer[j + 1] = b64_table[(triple >> 12) & 0x3F];
        result_buffer[j + 2] = b64_table[(triple >> 6) & 0x3F];
        result_buffer[j + 3] = b64_table[triple & 0x3F];
    }

    // Add padding
    pad_len = (3 - (data_len % 3)) % 3;
    for (i = 0; i < pad_len; i++) {
        result_buffer[out_len - 2 - i] = '=';
    }

    result_buffer[out_len - 1] = '\0';
    *result_len = out_len - 1; // Exclude null terminator
    return 1;
}
...
int from_base64_static(const unsigned char* data, size_t data_len,
    ...
    if (!data || !result_buffer || !result_len) {
        return 0;
    }
    ...
    // Calculate output size
    *result_len = (data_len / 4) * 3;
    if (data[data_len - 1] == '=') (*result_len)--;
    if (data[data_len - 2] == '=') (*result_len)--;

    if (*result_len > result_buffer_size) {
        return 0;
    }

    for (i = 0, j = 0; i < data_len; i += 4, j += 3) {
        sextet_a = b64_table[data[i]];
        sextet_b = b64_table[data[i + 1]];
        sextet_c = b64_table[data[i + 2]];
        sextet_d = b64_table[data[i + 3]];

        if (sextet_a == 64 || sextet_b == 64 ||
            (sextet_c == 64 && data[i + 2] != '=') ||
            (sextet_d == 64 && data[i + 3] != '=')) {
            return 0;
        }
        ...
        if (j < *result_len) result_buffer[j] = (triple >> 16) & 0xFF;
        if (j + 1 < *result_len) result_buffer[j + 1] = (triple >> 8) & 0xFF;
        if (j + 2 < *result_len) result_buffer[j + 2] = triple & 0xFF;
    }
