/*
 * Small Static Whitespace Steganography Test Program
 * 
 * This program demonstrates the small static C implementation with no dynamic allocation.
 * It uses pre-allocated buffers with a 1 MiB limit for testing.
 */
...
int main(int argc, char* argv[]) {
    printf("Small Static Whitespace Steganography Implementation\n");
    printf("Maximum buffer size: %zu bytes (%.2f MiB)\n", 
           whitespace_stego_static_small_max_size(),
           (double)whitespace_stego_static_small_max_size() / (1024.0 * 1024.0));
    ...
    // Test data
    const char* carrier = "Hello, this is a test message!";
    ...
    if (whitespace_stego_static_small_encode(&buffers, carrier, strlen(carrier), 
        ...
        if (whitespace_stego_static_small_decode(&buffers, buffers.encoded, encoded_len, 
                                                NULL, &decoded_len)) {
    ...
    if (whitespace_stego_static_small_validate_size(1024)) {
        printf("✓ 1024 bytes: Valid\n");
    } else {
        printf("✗ 1024 bytes: Invalid\n");
    }
    
    if (whitespace_stego_static_small_validate_size(whitespace_stego_static_small_max_size())) {
        printf("✓ Max size (%zu bytes): Valid\n", whitespace_stego_static_small_max_size());
    } else {
        printf("✗ Max size (%zu bytes): Invalid\n", whitespace_stego_static_small_max_size());
    }
    
    if (whitespace_stego_static_small_validate_size(whitespace_stego_static_small_max_size() + 1)) {
        printf("✗ Max size + 1: Valid (should be invalid)\n");
    } else {
        printf("✓ Max size + 1: Invalid (correct)\n");
    }
    
    printf("\nSmall static implementation test completed successfully!\n");
